...
class color{
    ...
    public:
        ...
        constexpr color get_int() const{
            ...
            std::uint8_t R = (r > 1.0) ? 255 : std::max(0, static_cast<int>(255*r));
            std::uint8_t B = (b > 1.0) ? 255 : std::max(0, static_cast<int>(255*b));
            std::uint8_t G = (g > 1.0) ? 255 : std::max(0, static_cast<int>(255*g));
            return {R, G, B};
        }
